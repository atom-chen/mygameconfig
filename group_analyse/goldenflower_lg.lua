...
local _ct_probability = {300, 260, 140, 120, 100, 80}
...
function logic.node1better(node1, node2)
	if node1.ct == node2.ct then
		if node1.kv ~= node2.kv then
			...
			elseif node1.ct == logic.ct_highcard then
				...
				for i = 1, 3 do
					if logic.getCardLogicValue(node1.cards[i]) ~= logic.getCardLogicValue(node2.cards[i]) then
						return node1.cards[i] > node2.cards[i]
					end
				end
				return -1
			else
	...
	for i = 1, 11 do
		...
		if hash[i] then
			...
			for j = i, i + _card_amount - 1 do
				if not hash[j] then
					if hash[1] and hash[2] and hash[13] then --A2345
						straightkv = 0
						break
					end
					for i1 = 13, 1, -1 do
