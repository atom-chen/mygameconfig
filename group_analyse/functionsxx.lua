...
local function getTraceback()
    local traceback = string.split(debug.traceback("", 5), "\n")
    local str = ""
...
function table:diySerializeTable(val, name, skipnewlines, depth)
    ...
    if type(val) == "table" then
        ...
        for k, v in pairs(val) do
            tmp =  tmp .. self:diySerializeTable(v, k, skipnewlines, depth + 1) .. "," .. (not skipnewlines and "\n" or "")
