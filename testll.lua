...
local _max_normal_play_sec = 500 --正常出牌的最大时间
...
local function get_delay_play_time(seatid)
	...
	if ds >= 2500 and ds < 4500 then
		return 1300
	elseif ds >= 4500 and ds < 9000 then
		return 900
	elseif ds >= 9000 then
		return 600
	end
...
ask_master = function(inorder)
	if ((_roomid == 1001 or isMatch or _personalgame) and score >= 3) or (not isMatch and not _personalgame and _roomid ~= 1001 and score >= 1) or _re_shuffle_count > 2 or (score > 0 and asktimes > 2)  then
		if _roomid ~= 1001 then
			score = 1
		end
	...
	elseif asktimes > 2 then
		...
		if isMatch then
			for k, v in pairs(players) do
				v.gold = v.gold - 100
				-- _ex_sub_gold[k] = _ex_sub_gold[k] or 0
				-- _ex_sub_gold[k] = _ex_sub_gold[k] - 100
			end
		end
		-- shuffle()
